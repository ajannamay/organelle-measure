...
for organelle in ["peroxisome","golgi","LD"]:
	for path in Path(f"images/preprocessed/{FOLDER6}/").glob(f"{nickname}_{organelle}*.tiff"):
		img = io.imread(str(path))
		img = (img>0.5)
		path_ref = path.parent / f"{path.stem.partition('_')[2]}.tif"
		ref = io.imread(str(path_ref))
...
for path in Path(f"images/preprocessed/{FOLDER6}").glob(f"{nickname}_mitochondria*.tiff"):
	img = io.imread(str(path))
	img = (img>0.5)
	output = measure.label(img)
	io.imsave(
		f"images/labelled/{FOLDER1}/{nickname}_{path.stem.partition('_')[2]}.tif",
		util.img_as_uint(output)
	)

# ER
path = Path(f"images/preprocessed/{FOLDER1}/{nickname}_preprocessed_ER_diploids1color.tiff")
img = io.imread(str(path))
img = (img>0.5)
ske = skeletonize_zbyz(img)
io.imsave(
	f"images/labelled/{FOLDER1}/{nickname}_ER_diploids1color.tif",
	util.img_as_uint(ske)
)

for path in Path(f"images/preprocessed/{FOLDER6}").glob(f"{nickname}_ER*.tiff"):
	img = io.imread(str(path))
	img = (img>0.5)
	ske = skeletonize_zbyz(img)
	io.imsave(
...
# # vacuole
# path = Path(f"images/preprocessed/{FOLDER1}/Probabilities_preprocessed_vacuole_diploids1color.tiff")
# img = io.imread(str(path))
# img = (img>0.5)
# path_cell = Path(f"images/cell/{FOLDER1}/camera-BF-after_EY2795triColor-EY2796WT_check-2.tif")
...
for path_cell in Path(f"images/cell/{FOLDER6}").glob(f"binCell_EYrainbow_glu-100_field-*.tif"):
	for organelle in organelles:
		path_organelle = Path(f"images/labelled/{FOLDER1}/{nickname}_{organelle}_{path_cell.stem.partition('_')[2]}.tif")
		...
		for cell in measure.regionprops(img_cell):
			...
			results.append(pd.DataFrame(measured))
		result = pd.concat(results,ignore_index=True)
		result.to_csv(f"data/{FOLDER1}/{path_organelle.stem}.csv",index=False)