# ~~Shankar thinks we should use the images directly from the original experiments. So data and plots generated by this script are not used.~~
# This was approved again at group meeting on March 22, after discussion with other group members.
# For new 6-color cells, field-4 is best for analyzing mitochondtria.
# And our ultimate goal is to let the discrepency between the 6-color and 1-color data as little as possible. Proposed solution includes: 
# - retrain 1-color ilastik to make it close to original 6-color
# - directly compare the already trained 1-color and new 6-color data
# - train ilastik with the 1-color and new 6-color together 
# - train ilastik with the 1-color and old 6-color together
# - train ilastik with 1-color, old and new 6-color together 
...
# %%
# Segment cells, only need to apply on new 6-color cells
...
# %%
# peroxisome
path = Path(f"images/raw/2024-02-16_rebuttal1color/all_unmixed-blue_diploid6color_field-3.nd2")
with ND2Reader(str(path)) as nd2:
	nd2.bundle_axes = "zyx"
	nd2.iter_axes   = "c"
	image = nd2[0]
...
io.imsave(
	f"images/preprocessed/rebuttal_diploid_v2/preprocessed_peroxisome_diploids6color_field-3.tif",
...
# %%
# ER
path = Path(f"images/raw/2024-02-16_rebuttal1color/all_spectra-green_diploid6color_field-3.nd2")
with ND2Reader(str(path)) as nd2:
	nd2.bundle_axes = 'zyx'
	nd2.iter_axes = "t"
	image = nd2[0]
...
io.imsave(
	f"images/preprocessed/rebuttal_diploid_v2/preprocessed_ER_diploids6color_field-3.tif",
	util.img_as_uint(gauss)
)
# %%
# golgi
path = Path(f"images/raw/2024-02-16_rebuttal1color/all_spectra-yellow_diploid6color_field-3.nd2")
with ND2Reader(str(path)) as nd2:
	nd2.bundle_axes = "zyx"
	nd2.iter_axes = 't'
	image = nd2[0]
gauss = filters.gaussian(image,sigma=0.75,preserve_range=True).astype(int)
io.imsave(
	f"images/preprocessed/rebuttal_diploid_v2/preprocessed_golgi_diploids6color_field-3.tif",
	util.img_as_uint(gauss)
)

# %%
# mitochondria
path_cell = Path(f"images/cell/2024-02-16_rebuttal1color/all_camera-after_diploid6color_field-3.tif")
cell = io.imread(str(path_cell))
path = Path(f"images/raw/2024-02-16_rebuttal1color/all_unmixed-red_diploid6color_field-3.nd2")
with ND2Reader(str(path)) as nd2:
	nd2.bundle_axes = "zyx"
	nd2.iter_axes = 'c'
	image = nd2[0]
...
io.imsave(
	f"images/preprocessed/rebuttal_diploid_v2/preprocessed_mitochondria_diploids6color_field-3.tif",
	util.img_as_uint(gauss)
)
# %%
...
gauss = filters.gaussian(clear,sigma=0.75,preserve_range=True).astype(int)
io.imsave(
	f"images/preprocessed/rebuttal_diploid_v2/preprocessed_LD_diploids6color_field-3.tif",
	util.img_as_uint(gauss)
)
# %%
...
# %%
# ER
for path in Path(f"images/preprocessed/rebuttal_diploid_v2/").glob("Probabilities*ER_*.tiff"):
	img = io.imread(str(path))
	img = (img>0.5)
...
for organelle in organelles:
	...
	path_organelle = Path(f"images/labelled/rebuttal_diploid_v2/{organelle}_diploids6color_field-3.tif")
...
# %%
# Comparison, NOT EXECUTED YET (2024-03-25: what does this mean? The plot scripy seems to have been executed.)
